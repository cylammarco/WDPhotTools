...
def althaus07_formatter():
    ...
    filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/althaus07/*.dat'))
...
def althaus09_formatter(mass_range='all'):
    ...
    filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/althaus09/z.*'))
...
def althaus17_formatter(model, mass_range='all'):
    ...
    if model == 'lpcode_co_db_17_z00005':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/althaus17/*d4.trk'))

    # Y=0.4, Z=0.0005 models
    if model == 'lpcode_co_db_17_z0001':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/althaus17/*d3.trk'))
...
def bedard20_formatter(model, mass_range='all'):
    ...
    if model == 'montreal_co_da_20':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/bedard20/*thick*'))

    # DB models
    if model == 'montreal_co_db_20':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/bedard20/*thin*'))
...
def lauffer18_formatter(model):
    ...
    if model == 'mesa_one_da_18':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/lauffer18/H_*.dat'))

    # He models
    if model == 'mesa_one_db_18':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/lauffer18/He_*.dat'))
...
def panei07_formatter(model):
    ...
    if model == 'lpcode_he_da_07':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/panei07/*He.SDSS'))

    # CO core models
    if model == 'lpcode_co_da_07':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/panei07/*CO.SDSS'))
...
def renedo10_formatter(model):
    ...
    if model == 'lpcode_co_da_10':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/renedo10/*z001.trk'))

    # Low metallicity model
    if model == 'lpcode_co_da_10_z0001':
        filelist = glob.glob(pkg_resources.resource_filename(
                    'WDLFBuilder', 'wd_cooling/renedo10/*z0001.trk'))
