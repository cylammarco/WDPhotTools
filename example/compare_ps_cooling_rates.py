...
Mbol = np.arange(6., 16., 0.02)
logL = np.log10(10.**((4.8 - Mbol) / 2.5) * 3.826e33)
...
ax1.tricontour(x[mask1],
               y[mask1],
               z1[mask1],
               levels=levels,
               linewidths=0.5,
               colors='k')
ax2.tricontour(x[mask2],
               y[mask2],
               z1[mask2],
               levels=levels,
               linewidths=0.5,
               colors='k')
ax3.tricontour(x[mask1 & mask2],
               y[mask1 & mask2], (z1 / z2)[mask1 & mask2],
               levels=levels,
               linewidths=0.5,
               colors='k')
# make the contour shades
contour1 = ax1.tricontourf(x[mask1],
                           y[mask1],
                           z1[mask1],
                           levels=levels,
                           cmap=cmap)
contour2 = ax2.tricontourf(x[mask2],
                           y[mask2],
                           z2[mask2],
                           levels=levels,
                           cmap=cmap)
contour3 = ax3.tricontourf(x[mask1 & mask2],
                           y[mask1 & mask2], (z1 / z2)[mask1 & mask2],
                           levels=levels,
                           cmap=cmap)
...
plt.subplots_adjust(wspace=0., left=0.075, right=0.975)
plt.savefig('example_output/compare_ps_cooling_rates.png')
